...
É inserida logo após todas as minhas mensagens. 

#+begin_src emacs-lisp
(setq mail-signature
      (format "\n\n-- \n%s - Estudante de Ciência da Computação" user-full-name))
#+end_src
...
#+begin_src sh :tangle ~/.config/isync/mbsyncrc :mkdirp yes :comments no
IMAPAccount Outlook
...
#+begin_src sh :tangle ~/.config/systemd/user/mbsync.service
[Unit]
